      ...
      <CardContent className="p-6">
        <div className="grid grid-cols-3 gap-4">
          {badges.map((badge) => (
            ...
            >
              <div
                className={`h-12 w-12 rounded-full ${badge.color} ${
                  badge.darkColor
                } flex items-center justify-center ${
                  !badge.locked ? "group-hover:scale-110 transition-transform" : ""
                }`}
              >
                <i className={`${badge.icon} text-xl ${badge.iconColor} ${badge.darkIconColor}`}></i>
              </div>
              <span className="mt-2 text-xs text-gray-500 dark:text-gray-400">{badge.name}</span>
            </div>
          ))}
        </div>
